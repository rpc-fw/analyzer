...
error_t ssiExecuteScript(HttpConnection *connection, const char_t *uri, uint_t level)
{
   ...
   buffer = osMemAlloc(HTTP_SERVER_BUFFER_SIZE);
   ...
   while(1)
   {
      ...
      if(more)
      {
         ...
         error = fsFileRead(file, buffer + pos + length,
   ...
   *separator = '\0';
   ...
   else
   {
      ...
      if(file)
      {
         ...
         {
            //Read data from the specified file
            error = fsFileRead(file, connection->buffer, HTTP_SERVER_BUFFER_SIZE, &length);
            //End of input stream?
            if(error) break;
