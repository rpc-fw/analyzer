...
error_t fsInit(void)
{
   ...
   //Mutex that protects critical sections
   fsMutex = osMutexCreate(FALSE);
   //Any error to report?
   if(fsMutex == OS_INVALID_HANDLE)
...
error_t fsFileGetSize(const char_t *path, uint32_t *size)
{
   FRESULT res;
   FILINFO fno;
...
error_t fsFileRename(const char_t *oldPath, const char_t *newPath)
{
   FRESULT res;
...
error_t fsFileDelete(const char_t *path)
{
   FRESULT res;
...
FsFile *fsFileOpen(const char_t *path, uint_t mode)
{
   uint_t i;
   uint_t flags;
   FRESULT res;
...
error_t fsFileSeek(FsFile *file, int_t offset, uint_t origin)
{
   FRESULT res;
...
error_t fsFileWrite(FsFile *file, void *data, size_t length)
{
   UINT n;
   FRESULT res;
...
error_t fsFileRead(FsFile *file, void *data, size_t size, size_t *length)
{
   UINT n;
   FRESULT res;
...
error_t fsDirCreate(const char_t *path)
{
   FRESULT res;
...
error_t fsDirRemove(const char_t *path)
{
   FRESULT res;
...
FsDir *fsDirOpen(const char_t *path)
{
   uint_t i;
   FRESULT res;
...
error_t fsDirRead(FsDir *dir, FsDirEntry *dirEntry)
{
   FRESULT res;
   FILINFO fno;
   char_t *fn;
